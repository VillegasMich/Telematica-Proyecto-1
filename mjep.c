...
void manage_chat(void *arg)
{
  ...
  while (1)
  {
    ...
    bzero((void *)header, BUFFER_SIZE_HEADER);
    bzero((void *)body, MAX_LEN_USERNAME * BACKLOG);
    int client_status = read_client(client_1, buff);
    if (client_status >= 0)
    {
      uncapsulate_client(buff, header, body);
      if ((strcmp(body, "exit\n")) == 0)
      {
        printf("Exit\n");
        break;
  ...
  else // connect with a user
  {
    ...
    for (;;)
    {
      ...
      if (client_array[i].chatting == index && i != index)
      {
        ...
        send_users(client_array, response, index);
        encapsulate_ack(response);
        send(client_array[index].socket, response,
             (MAX_LEN_USERNAME * BACKLOG) + BUFFER_SIZE_HEADER, 0);
        strcpy(response, "The other user was disconnected, please exit\n");
        encapsulate_message(response);
        send(client_array[i].socket, response,
             (MAX_LEN_USERNAME * BACKLOG) + BUFFER_SIZE_HEADER, 0);
        break;
